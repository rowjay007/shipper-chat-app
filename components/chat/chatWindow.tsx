...
export function ChatWindow() {
    ...
    const loadMessages = async () => {
      ...
      try {
        const response = await fetch(
          `/api/chat/messages?roomId=${selectedRoom.id}`,
        );
  ...
  if (!selectedRoom || !otherUser) {
    return (
      <div
        className="flex flex-1 flex-col bg-[#F3F3EE] overflow-hidden"
        style={{ maxHeight: "100%", height: "100%" }}
      >
        <div
          className="flex items-center justify-between border-b bg-white p-4 flex-shrink-0"
          style={{ height: "73px" }}
        >
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p
              className="text-xs"
              style={{ color: isOnline ? "#38C793" : "#9CA3AF" }}
            >
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            size="icon"
            className="bg-white border-[#E8E5DF]"
          >
          ...
          >
            <Info className="h-4 w-4 text-[#262626]" />
          </Button>
        </div>
      </div>
